...
public class CVRP{
  ...
  public void genetic_alg(){
    ...
    Chromosome c = new Chromosome(numNodes+NUM_VEHICLES-1);
  ...
  //System.out.println("Fitness START");
    double sum = 0;
    if (c.get(0)<=numNodes)  sum += data.getDistance(1, c.get(0));
    int i = 1;
    int node1 = 1;
    int node2 = 1;
    while(i<c.get_length()){
      if (c.get(i)>numNodes) node2 = 1;
      else node2 = c.get(i);
      if (c.get(i-1)>numNodes) node1 = 1;
      else node1 = c.get(i-1);
      sum+= data.getDistance(node1, node2);
  ...
  private void write(){
    try{
      ...
      pw.println("cost 3 decimal places");
      String str = "1";;
